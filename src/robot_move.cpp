...
#include "diffdrive_rpi/alphabot_cfg.h"
...
void RobotMove::config(int timerId1, int timerId2)
{
    this->motorLeft.config(PIN_MOTORL_EN,PIN_HBRIDGE_IN1,PIN_HBRIDGE_IN2,PIN_ENC_L1,PIN_ENC_L2,timerId1,MotCtrl_LEFT);
    this->motorRight.config(PIN_MOTORR_EN,PIN_HBRIDGE_IN4,PIN_HBRIDGE_IN3,PIN_ENC_R1,PIN_ENC_R2,timerId2,MotCtrl_RIGHT);
    this->motorLeft.start();
    this->motorRight.start();
}
...
void RobotMove::readEncoders(int* left, int* right)
{
    
    *left = this->motorLeft.getEnc();
    *right = this->motorRight.getEnc();
}