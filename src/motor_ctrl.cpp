#include "diffdrive_rpi/motor_ctrl.hpp"
#include <stdio.h>
#include <math.h>
...
#if defined(HW_N20D)
void MotorCtrl::config(const unsigned int PIN_PWM, const unsigned int PIN_IN1, const unsigned int PIN_IN2, const unsigned int PIN_ENC1, const unsigned int PIN_ENC2, const int timerNum, MotorCtrl_Side side)
#elif defined(HW_ORIGINAL)
void MotorCtrl::config(const unsigned int PIN_PWM, const unsigned int PIN_IN1, const unsigned int PIN_IN2, const unsigned int PIN_ENC1, const int timerNum)
#else
...
#endif
{
    ...
    this->enc_total = 0;
#if defined(HW_N20D)
    if(this->side_ == MotCtrl_LEFT)
    {
        this->enc.config(PIN_ENC1,PIN_ENC2,this->encoderICbkExt,this,true);
    }
    else
    {
        this->enc.config(PIN_ENC1,PIN_ENC2,this->encoderICbkExt,this,false);
    }
#elif defined(HW_ORIGINAL)
...
int MotorCtrl::getEnc(void)
{
    return this->enc_total;
}